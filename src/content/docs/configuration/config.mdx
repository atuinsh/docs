...
The history format allows you to config default history list format - which can also be specified with the --format arg.

```
The specified --format arg would prioritize the config when both existing
```
