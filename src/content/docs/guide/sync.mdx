...
In order use a self hosted server with Atuin, you'll have to set up the `sync_address` in the config file
at `~/.config/atuin/config.toml`. See [here](https://docs.atuin.sh/configuration/config/#sync_address) for
more details on how to set the `sync_address`.
